				...
				void *object)
{
	...
	kmsan_slab_setup_object(s, object);
	if (unlikely(s->ctor)) {
...
static struct page *allocate_slab(struct kmem_cache *s, gfp_t flags, int node)
{
	...
	///pr_err("allocated page %p with alloc_slab_page, order=%d, address=%p\n", page, oo_order(oo), page_address(page));
	if (unlikely(!page)) {
		oo = s->min;
		alloc_gfp = flags;
		/*
...
out:
	...
	BUG_ON(alloc_gfp & __GFP_NOTRACK);
	///kmsan_alloc_page(page, oo_order(oo), alloc_gfp);
	///kmsan_poison_slab(page, flags);
...
redo:
	...
	if (object) {
		...
		// TODO(glider)
			kmsan_unpoison_shadow(object, s->object_size);
		} else {
			// TODO(glider): need to call kmsan_poison_shadow() at most once.
			if (!s->ctor)
				kmsan_poison_shadow(object, s->object_size, gfpflags);
		}
	}
