...
static struct kcov_remote_area *kcov_remote_area_get(unsigned int size)
{
	...
	print_debug("kcov_remote_area_get: %u\n", size);
	...
	print_debug("kcov_remote_area_get = NULL\n");
	return NULL;
}
...
static void kcov_remote_area_put(struct kcov_remote_area *area,
					unsigned int size)
{
	print_debug("kcov_remote_area_put: %px %u\n", area, size);
...
{
	print_debug("kcov_start: t = %px, size = %u, area = %px\n",
			t, size, area);
...
static void kcov_remote_reset(struct kcov *kcov)
{
	...
	print_debug("kcov_remote_reset\n");

	spin_lock(&kcov_remote_lock);
	hash_for_each_safe(kcov_remote_map, bkt, tmp, remote, hnode) {
		if (remote->kcov != kcov)
			continue;
		print_debug("kcov_remote_reset: removing %llx\n",
				remote->handle);
...
void kcov_task_exit(struct task_struct *t)
{
	...
	print_debug("kcov_task_exit: t = %px, kcov->t = %px\n", t, kcov->t);
			     ...
			     unsigned long arg)
{
	...
	case KCOV_REMOTE_ENABLE:
		print_debug("KCOV_REMOTE_ENABLE\n");
		if (kcov->mode != KCOV_MODE_INIT || !kcov->area)
			return -EINVAL;
		t = current;
		if (kcov->t != NULL || t->kcov != NULL)
		...
		/* Put either in kcov_task_exit() or in KCOV_DISABLE. */
		kcov_get(kcov);
		print_debug("KCOV_REMOTE_ENABLE = 0\n");
		return 0;
	default:
		return -ENOTTY;
	}
...
void kcov_remote_start(u64 handle)
{
	...
	print_debug("kcov_remote_start: handle = %llx\n", handle);
	...
	if (!remote) {
		print_debug("kcov_remote_start: no remote => fail");
		spin_unlock(&kcov_remote_lock);
		return;
	}
	...
	print_debug("kcov_remote_start: task area: %px", area);
	spin_unlock(&kcov_remote_lock);

	if (!area) {
		print_debug("kcov_remote_start: vmalloc(%u)\n", size);
		area = vmalloc(size * sizeof(unsigned long));
		if (!area) {
			print_debug("kcov_remote_start: no area => fail");
	...
	*(u64 *)area = 0;
	...
	print_debug("kcov_remote_start = success\n");
				...
				unsigned int dst_area_size, void *src_area)
{
	...
	print_debug("kcov_move_area: %px %u <= %px %lu\n",
...
void kcov_remote_stop(void)
{
	...
	if (!kcov) {
		print_debug("kcov_remote_stop: no kcov => fail\n");
		return;
	}
	 ...
	 */
	print_debug("kcov_remote_stop: move if: %d == %d && %d\n",
	...
	print_debug("kcov_remote_stop = done\n");
}
